...
/**
 * SECTION:hildon-file-system-info
 * @short_description: API for querying info about files.
 *
 * #HildonFileSystemInfo provides a way to obtain information about a given
 * location (URI), such as its localized name and icon.
 */
...
hildon_file_system_info_async_cancel (HildonFileSystemInfoHandle *handle)
{
  ...
  if (info->cancellable)
  ...
  else
    {
      ...
      */
      g_warning ("attempt to cancel a completed "
 ...
 *
 * Gets icon for the location. All special places have their special
 * icons. Note that this function is just a special case of
